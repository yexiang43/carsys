...
<script type="text/javascript">
	...
	layui.use(['jquery', 'layer', 'form', 'table', 'laydate'], function () {
		...
		tableIns = table.render({
			...
			, url: '${chao}/customer/loadAllCustomer.action' //数据接口
			...
			, cols: [[   //列表数据
				...
				, {field: 'identity', title: '身份证号', align: 'center',width:'190'}
				, {field: 'phone', title: '电话号码', align: 'center',width: '150'}
				, {field: 'address', title: '客户地址', align: 'center',width: '150'}
				, {field: 'sex', title: '性别', align: 'center', width:'110',templet: function (d) {
		...
		$("#doSearch").click(function () {
			...
			//alert(params);
			tableIns.reload({
				url: "${chao}/customer/loadAllCustomer.action?" + params,
				page: {curr: 1}
			})
		});
		...
		table.on('tool(customerTable)', function (obj) {
			...
			if (layEvent === 'del') { //删除
				layer.confirm('真的删除【' + data.custname + '】这个客户么？', function (index) {
					//向服务端发送删除指令
					$.post("${chao}/customer/deleteCustomer.action", {identity: data.identity}, function (res) {
						layer.msg(res.msg);
						//刷新数据表格
						tableIns.reload();
					})
		...
		function openAddCustomer() {
			mainIndex = layer.open({
				...
				success: function (index) {
					//清空表单数据
					$("#dataFrm")[0].reset();
					url = "${chao}/customer/addCustomer.action";
				}
			});
		}
		...
		function openUpdateCustomer(data) {
			mainIndex = layer.open({
				...
				success: function (index) {
					...
					url = "${chao}/customer/updateCustomer.action";
				}
			});
		}
		...
		function deleteBatch() {
			...
			layer.confirm('真的要删除这些客户么？', function (index) {
				//向服务端发送删除指令
				$.post("${chao}/customer/deleteBatchCustomer.action", params, function (res) {
					layer.msg(res.msg);
					//刷新数据表格
					tableIns.reload();
				})
